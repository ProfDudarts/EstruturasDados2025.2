...
// Adicionados para garantir a análise correta do Intephense, mas devem ser 
// substituídos pelos arquivos reais (RbppavlCommon.php, NoArvoreAVL.php e um arquivo de constantes).
// =========================================================================

// --- 1. Constantes de Busca e Validação ---
...
if (!class_exists('Estruturas\NoArvoreAVL')) {
    class NoArvoreAVL {
        ...
        public function __construct($tree, $data, $parent) { 
            $this->data = $data; 
            $this->parent = $parent; 
        }
        // Métodos Mínimos Requeridos
        public function balance(): int { /* Lógica: altura direita - altura esquerda */ return 0; }
        public function heightReset(): void { /* Lógica para recalcular altura */ }
        public function wipe(&$ctr): void { /* Lógica para destruir subárvores */ $ctr++; }
        public function debugNodeValidate($balanceFactor, &$status) { return null; }
    }
}
...
if (!class_exists('Estruturas\RbppavlCommon')) {
    abstract class RbppavlCommon {
        ...
        protected function setStatus(int $id, array $params = null) {}
        protected function checkData($data, string $method): bool { return true; }
        protected function returnBytes(string $val): int { return 0; }
        protected function txt(string $key, array $params = null): string { return $key; }
        protected function nodeFind($tree, $data, &$y, &$q, &$dir, int $mode) { return null; }
    }
}
// =========================================================================
...
class ArvoreAVL extends RbppavlCommon 
{
     ...
     *
     * @param string $callbackClass classe da interface de callback a ser instanciada
    ...
    {
        // Lógica Completa de Rotação Restaurada:
    ...
    public function debugValidate($setStatusOnSuccess = false)
    {
        ...
        /** @var \Estruturas\NoArvoreAVL $root */
        $root = $this->root;

        $status = 0;
        $failingNode = $root->debugNodeValidate($this->balanceFactor, $status);
        if ($failingNode) {
            // validation failure
            $this->setStatus(
